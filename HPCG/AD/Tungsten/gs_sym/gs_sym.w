...
sp y         [z=NNZP] address(&x);   // y aliases x
...
iterator walkA3_bwd = ∀i24z A[BASE*26+24*ZL-1-i24z];
iterator walkA2_bwd = ∀i2z A[BASE*26+26*ZL-1-i2z];
...
iterator walkr_fwd =  ∀inf ∀zp ∀ip r[BASE+zp+ip*ZP];
iterator walky_fwd =  ∀inf ∀zp ∀ip y[BASEP+1+zp+ip*ZP];
iterator walkinvd_fwd =  ∀inf ∀zp ∀ip invd[BASE+zp+ip*ZP];

iterator walkZ2_bwd  = ∀inf ∀zp ∀ip ∀i2 x[BASEP+ZL+1-(zp+ip*ZP+2*i2)];
iterator walkZ3_bwd  = ∀inf ∀zp ∀ip ∀i3 x[BASEP+ZL+1-(zp+ip*ZP+i3)];
iterator walkZZ3_bwd = ∀inf ∀zp ∀ip ∀i3 x[BASEP+ZL+1-(zp+ip*ZP+1+i3)];
iterator walkr_bwd =  ∀inf ∀zp ∀ip r[BASE+ZL-1-(zp+ip*ZP)];
iterator walky_bwd =  ∀inf ∀zp ∀ip y[BASEP+1+ZL-1-(zp+ip*ZP)];
iterator walkinvd_bwd =  ∀inf ∀zp ∀ip invd[BASE+ZL-1-(zp+ip*ZP)];
...
if (1) {
  ...
  ∀ip {
    ∀i3 wv:tx_downstr[] ← x[BASEP+ZL+1-(ip*ZP+i3)]; // (new,old,old) upstream
  }
  ...
  ∀z {
    ...
    [[dot.load(off)]]
    ∀i2 acc = ∑ walkZ2_bwd[] *walkA2_bwd[]; //clears the accum
    [[dot.state(on), dot.load(off)]]
    {
        ∀i3 acc = ∑ (sp)(w[]) *walkA3_bwd[];
        ∀i3 acc = ∑ (sp)(se[])*walkA3_bwd[];
        ∀i3 acc = ∑ (sp)(s[]) *walkA3_bwd[];
        ∀i3 acc = ∑ (sp)(sw[])*walkA3_bwd[];
        ∀i3 acc = ∑ (sp)(ne[])*walkA3_bwd[];
        ∀i3 acc = ∑ (sp)(n[]) *walkA3_bwd[];
        ∀i3 acc = ∑ (sp)(nw[])*walkA3_bwd[];
    }
    [[dot.state(on)]]
    ∀i3 acc = ∑ (sp)(e[]) *walkA3_bwd[];
    rr[0] = (walkr_bwd[] - acc);
    walky_bwd[]   = rr[0] * walkinvd_bwd[];
    ∀i3 wv:tx_upstr[] ← walkZ3_bwd[]; // (new,new,old) downstream
    ∀i3 wv:tx_downstr[] ← walkZZ3_bwd[]; // next (new,old,old) upstream

  }

  ∀ip {
    ∀i3 nullmem[0]  ← (sp)(w[]) + zz[i3];  // discard
    ∀i3 nullmem[0]  ← (sp)(se[])  + zz[i3];
    ∀i3 nullmem[0]  ← (sp)(s[]) + zz[i3];
    ∀i3 nullmem[0]  ← (sp)(sw[])  + zz[i3];
  }
}
}
