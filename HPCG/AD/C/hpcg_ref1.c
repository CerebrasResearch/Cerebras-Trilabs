...
static void generate_problem(SparseMatrix *A, real_t *b, real_t *x, real_t *xexact) {
    ...
    for (int iz = 0; iz < nz; iz++) {
        ...
        for (int iy = 0; iy < ny; iy++) {
            ...
            for (int ix = 0; ix < nx; ix++) {
                ...
                int nnz = 0;
                real_t *vp = &A->values[row * MAX_NONZEROS_PER_ROW];
                global_int_t *gp = &A->colIndG[row * MAX_NONZEROS_PER_ROW];
...
static void generate_coarse_problem(int nranks,
    ...
    for (int r = 0; r < nranks; r++) {
        ...
        generate_problem(&Ac[r], NULL, NULL, NULL);
        setup_halo(&Ac[r]);
...
static real_t dot_all(int nranks, const local_int_t *nrow_arr, real_t **x, real_t **y) {
...
static void cg_all(int nranks,
    ...
    real_t rtz = (real_t)0.0, oldrtz = (real_t)0.0;
    real_t alpha = (real_t)0.0, beta = (real_t)0.0, pAp = (real_t)0.0;
    ...
    real_t rr = dot_all(nranks, nrow_arr, r_vec, r_vec);
...
int main(int argc, char **argv) {
    ...
    if (strcmp(g_test_mode, "hpcg") != 0 && strcmp(g_test_mode, "cg") != 0 &&
        ...
        fprintf(stderr, "Error: unknown test mode '%s' (use hpcg, cg, vcycle, spmv, gs_fwd, gs_bwd, or gs_sym)\n", g_test_mode);
        return 1;
    }
    ...
    for (int r = 0; r < nranks; r++) {
        ...
        generate_problem(&A[r], b_arr[r], x_arr[r], xexact_arr[r]);
        setup_halo(&A[r]);
    }
